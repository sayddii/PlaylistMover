...
LANG = {
    "en": {
        ...
        "search": "Found {0} tracks. Searching...",
        "not_found_file": "YouTube JSON file not found",
        "browse": "Browse",
        "err_quota": "⛔ YOUTUBE QUOTA EXCEEDED!\nGoogle allows adding ~60 songs per day.\nTry again tomorrow or create a new Google Cloud project.",
...
1. SPOTIFY (Get ID and Secret):
   ...
   - Click Save.
   - Copy "Client ID" and "Client Secret" into this program's settings.
...
2. YOUTUBE (Get JSON file):
   ...
   - In this program's settings click "Browse" and select that file.

3. START:
   - Save settings.
   - Click "Connect Accounts".
   - Login to Spotify and Google (Allow access).
"""
    },
    "ru": {
        ...
        "search": "Найдено {0} треков. Поиск...",
        "not_found_file": "Не найден файл JSON для YouTube",
        "browse": "Обзор",
        "err_quota": "⛔ ЛИМИТ YOUTUBE ИСЧЕРПАН!\nGoogle разрешает добавлять около 60 песен в день.\nПопробуйте завтра или создайте новый проект.",
    ...
    "ua": {
        ...
        "search": "Знайдено {0} треків. Пошук...",
        "not_found_file": "Не знайдено файл JSON для YouTube",
        "browse": "Огляд",
        "err_quota": "⛔ ЛІМІТ YOUTUBE ВИЧЕРПАНО!\nGoogle дозволяє додавати близько 60 пісень на день.\nСпробуйте завтра або створіть новий проект.",
...
def tr(key):
    cfg = load_config()
    lang_code = cfg.get("lang", "en") # По умолчанию берем en, если нет в конфиге
...
class MusicLogic:
    ...
    def update_config(self): self.config = load_config()
    ...
    def connect(self):
        ...
        try:
            ...
            self.sp = spotipy.Spotify(auth_manager=SpotifyOAuth(
                client_id=self.config["sp_id"], client_secret=self.config["sp_secret"],
                redirect_uri=self.config["sp_redirect"], scope="playlist-read-private playlist-modify-public playlist-modify-private"))
            ...
            self.log(f"{tr('log_err')}{e}"); self.log(tr("log_err_keys"))
            return False
    ...
    def _auth_youtube(self):
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token: creds.refresh(Request())
                ...
                flow = InstalledAppFlow.from_client_secrets_file(self.config["yt_json_path"], ['https://www.googleapis.com/auth/youtube.force-ssl'])
    ...
    def get_user_playlists(self, service):
        ...
        # ОБРАБОТКА ОШИБКИ КВОТЫ ПРИ ЗАГРУЗКЕ
    ...
    def _similarity(self, a, b): return SequenceMatcher(None, a, b).ratio()
        ...
        if mode == 'yt_to_sp':
            ...
            try:
                while True:
                    ...
                    for item in resp['items']:
                        videos.append({'title': item['snippet']['title'], 'channel': item['snippet'].get('videoOwnerChannelTitle', item['snippet']['channelTitle'])})
                    next_page = resp.get('nextPageToken')
                    if not next_page: break
            except Exception as e: self.log(f"❌ YouTube Error: {e}"); return
            ...
            for vid in videos:
                ...
                if " - " in raw_t:
                    ...
                    artist_g = clean_c; track_g = self._clean_string(raw_t)
            ...
            if uris:
                try:
                    ...
                    for i in range(0, len(uris), 100): self.sp.playlist_add_items(pl['id'], uris[i:i+100])
        ...
        # === SPOTIFY -> YOUTUBE ===
            ...
            try:
                new_pl = self.yt.playlists().insert(part='snippet,status', body={
                    'snippet': {'title': f"PlaylistMover: {playlist_name}"}, 'status': {'privacyStatus': 'private'}
                }).execute()
                new_pl_id = new_pl['id']
            except HttpError as e:
                if "quotaExceeded" in str(e): self.log("\n" + tr("err_quota")); return
                else: self.log(f"❌ YouTube Error: {e}"); return
            except Exception as e: self.log(f"Error: {e}"); return
...
class SettingsWindow(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        self.title(tr("title_settings")); self.geometry("500x550"); self.parent = parent
        self.tabview = ctk.CTkTabview(self, width=480, height=480); self.tabview.pack(padx=10, pady=10)
        self.tab_keys = self.tabview.add(tr("tab_keys")); self.tab_lang = self.tabview.add(tr("tab_lang")); self.tab_help = self.tabview.add(tr("tab_help"))
        config = load_config()

        ctk.CTkLabel(self.tab_keys, text="Spotify Client ID:").pack(anchor="w", padx=10)
        self.entry_sp_id = ctk.CTkEntry(self.tab_keys, width=400); self.entry_sp_id.insert(0, config.get("sp_id", "")); self.entry_sp_id.pack(padx=10, pady=(0, 10))
        ctk.CTkLabel(self.tab_keys, text="Spotify Client Secret:").pack(anchor="w", padx=10)
        self.entry_sp_secret = ctk.CTkEntry(self.tab_keys, width=400, show="*"); self.entry_sp_secret.insert(0, config.get("sp_secret", "")); self.entry_sp_secret.pack(padx=10, pady=(0, 10))
        ctk.CTkLabel(self.tab_keys, text="Spotify Redirect URI:").pack(anchor="w", padx=10)
        self.entry_sp_uri = ctk.CTkEntry(self.tab_keys, width=400); self.entry_sp_uri.insert(0, config.get("sp_redirect", "http://127.0.0.1:8888/callback")); self.entry_sp_uri.pack(padx=10, pady=(0, 10))
        ctk.CTkLabel(self.tab_keys, text="YouTube JSON:").pack(anchor="w", padx=10)
        self.frame_yt = ctk.CTkFrame(self.tab_keys, fg_color="transparent"); self.frame_yt.pack(padx=10, pady=(0, 10), fill="x")
        self.entry_yt_path = ctk.CTkEntry(self.frame_yt, width=300); self.entry_yt_path.insert(0, config.get("yt_json_path", "")); self.entry_yt_path.pack(side="left")
        ...
        help_box = ctk.CTkTextbox(self.tab_help, width=450, height=400); help_box.pack(pady=5); 
        help_box.insert("0.0", tr("help_text")); 
        help_box.configure(state="disabled")

    def browse_file(self):
        f = filedialog.askopenfilename(filetypes=[("JSON files", "*.json")]); 
        if f: self.entry_yt_path.delete(0, "end"); self.entry_yt_path.insert(0, f)

    def save_settings(self):
        save_config({"sp_id": self.entry_sp_id.get().strip(), "sp_secret": self.entry_sp_secret.get().strip(), "sp_redirect": self.entry_sp_uri.get().strip(), "yt_json_path": self.entry_yt_path.get().strip(), "lang": self.lang_var.get()})
        messagebox.showinfo(tr("saved"), tr("saved")); self.parent.refresh_ui_text(); self.destroy()

class SSK4SApp(ctk.CTk):
    def __init__(self):
        super().__init__(); self.title("PlaylistMover"); self.geometry("600x650")
        ctk.set_appearance_mode("Dark"); ctk.set_default_color_theme("blue")
        self.col_sky = "#00A8E8"; self.col_sky_hover = "#4CC9F0"; self.text_on_sky = "#1a1a1a"
        self.logic = MusicLogic(self.log_message); self.yt_playlists = {}; self.sp_playlists = {}
        # ВНИМАНИЕ: Переименуй ssk4s.ico в PlaylistMover.ico
        self.iconbitmap("PlaylistMover.ico") 

        # ИЗМЕНЕНО: ИДЕАЛЬНАЯ ЦЕНТРОВКА
        self.header_frame = ctk.CTkFrame(self, height=60, fg_color="transparent")
        self.header_frame.pack(pady=20, fill="x")
        
        # Заголовок строго по центру окна (anchor="center" и relx=0.5)
        ...
        self.btn_connect = ctk.CTkButton(self, text=tr("connect"), fg_color=self.col_sky, hover_color=self.col_sky_hover, text_color=self.text_on_sky, height=40, font=("Arial", 14, "bold"), command=self.start_connect); self.btn_connect.pack(pady=10)
        self.lbl_src = ctk.CTkLabel(self, text=tr("source_lbl")); self.lbl_src.pack(pady=(20, 5))
        self.source_var = ctk.StringVar(value="YouTube")
        self.seg_source = ctk.CTkSegmentedButton(self, values=["YouTube", "Spotify"], variable=self.source_var, command=self.on_source_change, selected_color=self.col_sky, selected_hover_color=self.col_sky_hover); self.seg_source.pack(pady=5)
        self.combo_var = ctk.StringVar(value=tr("combo_wait")); self.combo_playlist = ctk.CTkComboBox(self, width=400, variable=self.combo_var, state="disabled"); self.combo_playlist.pack(pady=10)
        self.btn_start = ctk.CTkButton(self, text=tr("btn_to_sp"), fg_color=self.col_sky, hover_color=self.col_sky_hover, text_color=self.text_on_sky, height=50, width=250, font=("Arial", 16, "bold"), state="disabled", command=self.start_transfer); self.btn_start.pack(pady=20)
        self.textbox = ctk.CTkTextbox(self, width=550, height=200, border_color="#333", border_width=2); self.textbox.pack(pady=10); self.textbox.configure(state="disabled")
        self.log_message(tr("welcome"))

    def refresh_ui_text(self):
        self.btn_connect.configure(text=tr("connect")); self.lbl_src.configure(text=tr("source_lbl"))
        self.combo_var.set(tr("combo_wait")); self.on_source_change(self.source_var.get())
        self.log_message("-" * 20); self.log_message(tr("welcome"))

    def log_message(self, message):
        self.textbox.configure(state="normal"); self.textbox.insert("end", message + "\n"); self.textbox.see("end"); self.textbox.configure(state="disabled")
    def open_settings(self): SettingsWindow(self)
    def start_connect(self): threading.Thread(target=self._connect_thread, daemon=True).start()
    def _connect_thread(self):
        if self.logic.connect():
            self.yt_playlists = self.logic.get_user_playlists('youtube'); self.sp_playlists = self.logic.get_user_playlists('spotify')
            self.btn_connect.configure(state="disabled", text=tr("connected"), fg_color="#333", text_color="white")
            self.combo_playlist.configure(state="normal"); self.btn_start.configure(state="normal")
    ...
    def on_source_change(self, value):
        ...
        if value == "YouTube":
            names = list(self.yt_playlists.keys()); self.combo_playlist.configure(values=names) if names else None
            self.combo_var.set(names[0] if names else tr("combo_empty")); self.btn_start.configure(text=tr("btn_to_sp"))
        else:
            names = list(self.sp_playlists.keys()); self.combo_playlist.configure(values=names) if names else None
            self.combo_var.set(names[0] if names else tr("combo_empty")); self.btn_start.configure(text=tr("btn_to_yt"))
    def start_transfer(self):
        pl_name = self.combo_var.get(); source = self.source_var.get()
        ...
        if not pl_id: self.log_message(f"❌ {tr('combo_empty')}"); return
    ...
    def _run_logic_wrapper(self, mode, pl_id, pl_name):
        ...
        self.btn_start.configure(state="normal", text=tr("btn_to_sp") if self.source_var.get() == "YouTube" else tr("btn_to_yt"), fg_color=self.col_sky, text_color=self.text_on_sky)

if __name__ == "__main__": app = SSK4SApp(); app.mainloop()